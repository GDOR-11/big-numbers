...
fn main() {
    ...
    let number = args.get(1).expect("1 argument is required to run the program").parse::<u64>().expect("First argument must be a number!");
    let save_interval = args.get(2).map(|arg| arg.parse::<u64>().expect("Second argument must be a number, if present"));


    let closest_calculated_number = get_closest_calculated_number(number, DIRECTORY);
    println!("Calculating {number}!, starting from {}!", closest_calculated_number.0);
    ...
    for x in closest_calculated_number.0 + 1..=number {
    ...
    save_factorial(number, &factorial, DIRECTORY).expect(&format!("Could not save file '{}'", filepath(DIRECTORY, number)));
}
